...
class BNLSTMCell(RNNCell):
    ...
    def _batch_norm(self, x, name_scope, step, epsilon=1e-5, no_offset=False, set_forget_gate_bias=False):
        ...
        with tf.compat.v1.variable_scope(name_scope):
            ...
            scale = tf.compat.v1.get_variable('scale', [size], initializer=tf.compat.v1.constant_initializer(self._initial_scale))
            if no_offset:
                offset = 0
            elif set_forget_gate_bias:
                offset = tf.compat.v1.get_variable('offset', [size], initializer=offset_initializer())
            else:
                offset = tf.compat.v1.get_variable('offset', [size], initializer=tf.compat.v1.zeros_initializer)

            pop_mean_all_steps = tf.compat.v1.get_variable('pop_mean', [self._max_bn_steps, size], initializer=tf.compat.v1.zeros_initializer, trainable=False)
            pop_var_all_steps = tf.compat.v1.get_variable('pop_var', [self._max_bn_steps, size], initializer=tf.compat.v1.ones_initializer(), trainable=False)
            ...
            return tf.cond(pred=self._training, true_fn=batch_statistics, false_fn=population_statistics)
