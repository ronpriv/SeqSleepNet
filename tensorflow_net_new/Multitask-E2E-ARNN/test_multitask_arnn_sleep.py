    ...
    with sess.as_default():
        ...
        def dev_step(x_batch, y_batch):
            ...
            output_loss, total_loss, yhat, score = sess.run(
                   [arnn.output_loss, arnn.loss, arnn.predictions, arnn.scores], feed_dict)
        ...
        def evaluate(gen):
            ...
            while test_step < num_batch_per_epoch:
                ...
                for n in range(config.epoch_seq_len):
                    ...
                    score[n, (test_step-1)*config.batch_size : test_step*config.batch_size,:] = score_[n]
                test_step += 1
            if(gen.pointer < len(gen.data_index)):
                ...
                for n in range(config.epoch_seq_len):
                    ...
                    score[n, (test_step-1)*config.batch_size : len(gen.data_index),:] = score_[n]
                output_loss += output_loss_
                total_loss += total_loss_
            yhat = yhat + 1
            ...
            for n in range(config.epoch_seq_len):
                test_acc[n] = accuracy_score(yhat[n,:], gen.label[gen.data_index - int(np.floor(config.epoch_seq_len/2)) + n]) # due to zero-indexing
