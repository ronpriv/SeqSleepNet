...
from multitask_arnn_sleep import Multitask_ARNN_Sleep
from multitask_arnn_sleep_config import Config
...
tf.compat.v1.app.flags.DEFINE_string("eeg_eval_data", "../data/eval_data_1.mat", "Point to directory of input data")
...
tf.compat.v1.app.flags.DEFINE_string("emg_eval_data", "../data/eval_data_1.mat", "Point to directory of input data")
...
tf.compat.v1.app.flags.DEFINE_integer("seq_len", 3, "Sequence length (default: 32)")

tf.compat.v1.app.flags.DEFINE_integer("nfilter", 20, "Sequence length (default: 20)")

tf.compat.v1.app.flags.DEFINE_integer("nhidden1", 64, "Sequence length (default: 20)")
tf.compat.v1.app.flags.DEFINE_integer("attention_size1", 32, "Sequence length (default: 20)")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        arnn = Multitask_ARNN_Sleep(config=config)
        ...
        def dev_step(x_batch, y_batch):
            ...
            output_loss, total_loss, yhat = sess.run(
                   [arnn.output_loss, arnn.loss, arnn.predictions], feed_dict)
        ...
        def evaluate(gen, log_filename):
            ...
            with open(os.path.join(out_dir, log_filename), "a") as text_file:
                ...
                for n in range(config.epoch_seq_len):
                    acc_n = accuracy_score(yhat[n,:], gen.label[gen.data_index.astype(int) - int(np.floor(config.epoch_seq_len/2) + n)]) # due to zero-indexing
