...
def bidirectional_recurrent_layer(nhidden, nlayer, input_keep_prob=1.0, output_keep_prob=1.0):
    fw_cell = tf.compat.v1.nn.rnn_cell.GRUCell(nhidden)
    bw_cell = tf.compat.v1.nn.rnn_cell.GRUCell(nhidden)
    if (nlayer > 1):
        fw_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([fw_cell] * nlayer,
                                              state_is_tuple=True)  # due to built with LN LSTM
        bw_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([bw_cell] * nlayer,
                                              state_is_tuple=True)  # due to built with LN LSTM
...
def bidirectional_recurrent_layer_output(fw_cell, bw_cell, input_layer, sequence_len, scope=None):
    ((fw_outputs,
      bw_outputs),
     (fw_state,
      bw_state)) = (tf.compat.v1.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell,
                                                    cell_bw=bw_cell,
                                                    inputs=input_layer,
...
def attention(inputs, attention_size, time_major=False):
    ...
    alphas = exps / tf.reshape(tf.reduce_sum(input_tensor=exps, axis=1), [-1, 1])

    # Output of Bi-RNN is reduced with attention vector
    output = tf.reduce_sum(input_tensor=inputs * tf.reshape(alphas, [-1, sequence_length, 1]), axis=1)
